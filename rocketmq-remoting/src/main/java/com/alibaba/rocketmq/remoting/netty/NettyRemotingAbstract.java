...
public abstract class NettyRemotingAbstract {
    ...
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        ...
        if (pair != null) {
            Runnable run = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        // 包含设置 kv config配置，获取kv config,删除kv config,注册broker,
                        // 下线broker,根据Topic获取Broker Name、队列数,获取注册到Name Server的所有Broker集群信息
            ...
            try {
                //消息实际的处理并不是在当前线程，而是被封装成task放到Processor对应的线程池处理
